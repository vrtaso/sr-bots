...
- заявителю приходит уведомление о решении;
- при одобрении — объявление о новом участнике в общий чат клана (CLAN_CHAT_ID).
...
Ограничение: список анкет, ожидающих решения модератора, хранится в
памяти процесса. Если бот перезапустится до того, как модератор нажал
кнопку — по уже отправленной анкете кнопки перестанут работать (бот
сообщит об этом при попытке нажать), нужно будет подать анкету заново.
"""
...
# app_id -> данные анкеты, ожидающей решения модератора
PENDING_APPLICATIONS: dict[str, dict] = {}
...
async def get_timezone(message: Message, state: FSMContext):
    ...
    try:
        ...
        PENDING_APPLICATIONS.pop(app_id, None)
        await message.answer(
...
async def handle_decision(callback: CallbackQuery):
    ...
    application = PENDING_APPLICATIONS.pop(app_id, None)

    if application is None:
        await callback.answer(
            "Эта анкета уже обработана или недоступна (например, бот перезапускался).",
            show_alert=True,
        )
        return
    ...
    try:
        if approved:
            ...
            if CLAN_INVITE_LINK:
                text += f"\n\nСсылка на общий чат клана: {CLAN_INVITE_LINK}"
        else:
            text = (
    ...
    # При одобрении — объявление в общий чат клана.
    if approved:
        try:
            announcement = (
                f"🎉 В клан {CLAN_NAME} принят новый участник!\n\n"
                f"👤 {application['name']} ({application['username']})\n"
                f"⭐ Уровень: {application['level']}\n"
                f"🌍 Часовой пояс: {application['timezone']}\n\n"
                f"Добро пожаловать! 🖤"
            )
            await bot.send_message(chat_id=CLAN_CHAT_ID, text=announcement)
        except Exception:
            log.exception("Не удалось отправить уведомление в чат клана")
